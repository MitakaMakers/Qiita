...
フーリエ変換には、連続時間信号に適用される ①フーリエ級数展開 と② 狭義のフーリエ変換、離散時間信号に適用される③ 離散時間フーリエ変換と④離散フーリエ変換(DFT）の4つの種類があります。工学分野・産業用途では離散フーリエ変換をその規則性を利用して高速化した⑤高速フーリエ変換（FFT）が主に使われます。
