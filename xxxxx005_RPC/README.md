...
|浮動小数点型|4 バイト|
|倍精度浮動小数点型|8 バイト|
|固定長のデータ型|全体のバイト数が 4 の倍数でない場合、末尾に余計な0値が付きます|
|可変長のデータ型|長さを表現する4バイトのシーケンスが先頭に付きます。全全体のバイト数が 4 の倍数でない場合、末尾に余計な0値が付きます|
|文字列|長さを表現する4バイトのシーケンスが先頭に付きます。全体のバイト数が 4 の倍数でない場合、末尾に余計な0値が付きます|
...
+ LAST_FLAG：通信経路が TCP の場合のみつきます。呼び出しデータが複数のメッセージに分割されているかを示します。
+ flag_header：通信経路が TCP の場合のみつきます。１つのメッセージの長さを示します。
...
+ prog, vers, proc：prog（プログラム番号）, vers（バージョン番号）, proc（プロシージャ番号） の組で実行する関数を示します
